...
class AmclNode
{
  ...
  private:
    ...
    std::vector< bool > lasers_update_; // 是否需要更新（是否需要对当前帧激光数据进行处理）
    std::map< std::string, int > frame_to_laser_; // <雷达传感器、在列表中的索引>

    // Particle filter
    pf_t *pf_;
    double pf_err_, pf_z_;
    bool pf_init_; // 是否是第一帧，true表示不是第一帧，flase表示是第一帧；更新全局地图或初始位姿之后为第一帧
    pf_vector_t pf_odom_pose_; // 上一时刻的位姿
...
/// 根据输入的OccupancyGrid消息，对滤波器进行初始化
/// 分别构建了 地图对象、里程计对象、雷达传感器对象
void
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  int laser_index = -1; // 雷达传感器在列表中的索引

  // 是否已经接受过该雷达传感器的消息
  // Do we have the base->base_laser Tx yet?
  if(frame_to_laser_.find(laser_scan->header.frame_id) == frame_to_laser_.end())
  {
    /// 从未接收过该雷达传感器的消息，加入到列表中
  ...
  pf_vector_t pose; // 当前时刻的里程计位姿 x y yaw
  ...
  if(pf_init_)
  {
    ...
    // 运动幅度是否超过设定值，超过的话则需要进行更新
    // See if we should update the filter
    bool update = fabs(delta.v[0]) > d_thresh_ ||
                  fabs(delta.v[1]) > d_thresh_ ||
                  fabs(delta.v[2]) > a_thresh_;
    update = update || m_force_update; // 是否强制更新
    m_force_update=false; // 下次不再强制更新
  ...
  bool resampled = false; // 粒子是否进行了重采样
  // If the robot has moved, update the filter
  if(lasers_update_[laser_index])
  {
    /// 需要更新（需要对当前帧数据进行处理）
      
    // 激光数据 格式转换为需要的格式 AMCLLaserData
    AMCLLaserData ldata;
    ...
    if (!m_force_update) 
    {
      ...
      particlecloud_pub_.publish(cloud_msg); // 发布粒子云
    }
  }
  ...
  {
    /// 进行了重采样，发布机器人的后验位姿
    /// 遍历所有粒子簇，找出权重最大的簇，其平均位姿就是我们要求的机器人后验位姿  
    ...
    if(max_weight > 0.0)
    {
      ...
      if (tf_broadcast_ == true)
      {
        ...
        this->tfb_->sendTransform(tmp_tf_stamped); // 发布 map_to_odom Transform
