...
////////////////////////////////////////////////////////////////////////////////
// Insert a pose into the tree. value 为粒子的权重
void pf_kdtree_insert(pf_kdtree_t *self, pf_vector_t pose, double value)
{
  int key[3];

  // pose 转化为 Cell 的 keys
                                        ...
                                        pf_kdtree_node_t *node, int key[], double value)
{
  ...
  else /// 3.非空非叶，递归遍历
  {
    ...
    if (key[node->pivot_dim] < node->pivot_value)
      pf_kdtree_insert_node(self, node, node->children[0], key, value); // 向左寻找
    else
      pf_kdtree_insert_node(self, node, node->children[1], key, value); // 向右寻找
  }
...
void pf_kdtree_cluster_node(pf_kdtree_t *self, pf_kdtree_node_t *node, int depth)
{
  ...
  for (i = 0; i < 3 * 3 * 3; i++)
  {
    ...
    nnode->cluster = node->cluster; // 标记为当前簇
