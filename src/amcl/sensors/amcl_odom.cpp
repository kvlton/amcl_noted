...
bool AMCLOdom::UpdateAction(pf_t *pf, AMCLSensorData *data)
{
  ...
  pf_vector_t old_pose = pf_vector_sub(ndata->pose, ndata->delta); // 上一时刻的里程计位姿
  ...
  /// 里程计模型 sample_motion_model_odometry
  case ODOM_MODEL_DIFF:
  {
    ...
    /// 遍历所有粒子，利用运动模型计算每个粒子的先验位姿
    for (int i = 0; i < set->sample_count; i++)
    {
      pf_sample_t* sample = set->samples + i; // 单个粒子

      // Sample pose differences
      /// 2.旋转δ̂ rot1、平移δ̂ trans、旋转δ̂ rot2
