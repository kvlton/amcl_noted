...
double AMCLLaser::BeamModel(AMCLLaserData *data, pf_sample_set_t* set)
{
  ...
  {
    sample = set->samples + j; // 当前粒子
    pose = sample->pose; // 机器人的位姿
    ...
    {
      // 单个激光束
      obs_range = data->ranges[i][0];   // 距离（测量距离）
      obs_bearing = data->ranges[i][1]; // 方位

      // 根据地图，计算“真实”的range（真实距离）
      ...
      z = obs_range - map_range; // 测量距离 - 真实距离
    ...
    sample->weight *= p; // 单个粒子的权重
    total_weight += sample->weight; // 粒子集的累计权重
  }

  return(total_weight); // 返回粒子集的累计权重
}
...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set)
{
  ...
  for (j = 0; j < set->sample_count; j++)
  {
    ...
    for (i = 0; i < data->range_count; i += step)
    {
      ...
      obs_bearing = data->ranges[i][1]; // 方向
