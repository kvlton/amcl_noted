...
{
  // Depth in the tree
  int leaf, depth; // leaf：是否为叶子节点

  // Pivot dimension and value
  int pivot_dim; // 主维度：x y θ 中差异最大的维度
  double pivot_value; // 主维度对应的key值：左右子节点key的平均值

  // The key for this node
  int key[3]; // x y θ 对应的 key
  ...
  // Child nodes
  struct pf_kdtree_node *children[2]; // left right
...
{
  // Cell size
  double size[3]; // 一个立方体Cell的大小 50cm x 50cm x 10°
