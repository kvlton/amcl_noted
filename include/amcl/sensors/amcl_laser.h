...
typedef enum
{
  ...
  LASER_MODEL_LIKELIHOOD_FIELD,     // 似然场模型
  LASER_MODEL_LIKELIHOOD_FIELD_PROB // 似然场模型（波束过滤）
} laser_model_t;
...
class AMCLLaserData : public AMCLSensorData
{
  ...
  public: int range_count; // 波束数量 360
  public: double range_max; // 最大量程
  public: double (*ranges)[2]; // <距离，方位>
};
...
class AMCLLaser : public AMCLSensor
{
  ...
  private: pf_vector_t laser_pose; // 外参 laser_to_base
  
  // Max beams to consider
  // 测量模型不会使用所有激光束，仅使用部分（360->30）
  private: int max_beams; // 30
